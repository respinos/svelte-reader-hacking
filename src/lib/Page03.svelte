<script>
  ...
  const doStuff = ({detail}) => {
    ...
    let img_src = `/cgi/imgsrv/image?id=${canvas.id}&seq=${seq}&height=${scanHeight}`;
    fetch(img_src)
      .then((response) => {
  ...
  onMount(() => {
    testWidth = canvas.originalWidth || canvas.width;
    testHeight = canvas.originalHeight || canvas.height;
...
<div class="page" id="id{seq}" class:visible={isVisible} use:observer on:intersecting="{doStuff}" on:unintersecting={undoStuff}>
  <div class="test-frame" style="--width: {testWidth}px; --height: {testHeight}px;"></div>
  <span>Item {seq}</span>
